...
class EnergyScan(seamm.Node):
    ...
    def __init__(
        ...
        flowchart=None,
        title="Energy Scan",
        namespace="org.molssi.seamm.energy_scan",
        extension=None,
        logger=logger
    ):
    ...
    def description_text(self, P=None):
        ...
        while node is not None:
            try:
                ...
                print(
                    "Unexpected error describing energy_scan flowchart: {} in {}"
    ...
    def run(self):
        ...
        result = local.run(
            cmd=["Energy Scan", "-in", "molssi.dat"],
            files=files,
            return_files=[]
        )  # yapf: disable

        if result is None:
            logger.error("There was an error running Energy Scan")
            return None
